...
if ($has_wc && $is_current_tab) :
	...
	$plan_start = isset($this->options['account_info']['plan']['start_datetime']) ? (new DateTime($this->options['account_info']['plan']['start_datetime']))->format('d/m/Y') : null;
	$plan_end = isset($this->options['account_info']['plan']['end_datetime']) ? (new DateTime($this->options['account_info']['plan']['end_datetime']))->format('d/m/Y') : null;
	$available_coins = isset($this->options['account_info']['available_coins']) ? $this->options['account_info']['available_coins'] : 0;
	$coins_status = $available_coins < 3 ? 'expired' : ($available_coins < 6 ? 'expiring' : 'ok');
	$upgrade_url = Spoki()->get_plan_link(true);

	if (isset($available_coins) && $coins_status != 'ok') { ?>
	...
	if (isset($this->options['account_info'])) :
...
<br/><br/>Respond quickly and comfortably to your potential customers and take advantage of the WhatsApp communication channel to offer them promotions and <b>push them to purchase</b>!</p>', 'spoki') ?>
            ...
            <div class="spoki-account-overview spoki-card">
                ...
                <div class="body spoki-card-body">
                    ...
                    <div class="spoki-statistic">
                        <span class="coins">
                            <?php echo $this->options['account_info']['active_contacts'] ?>
                        </span>
                        <div>
                ...
                <div class="footer spoki-card-footer">
                    <span>
                        <?php
						if (isset($this->options['account_info']['plan'])) {
							$price = ($this->options['account_info']['plan']['standard_amount'] != $this->options['account_info']['plan']['amount']) ?
								'<del aria-hidden="true">' . wc_price( $this->options['account_info']['plan']['standard_amount'] / 100, ['currency' => 'EUR'] ) . '</del> <ins>' . wc_price( $this->options['account_info']['plan']['amount'] / 100, ['currency' => 'EUR'] ) . '</ins>' :
								wc_price($this->options['account_info']['plan']['amount'] / 100, ['currency' => 'EUR']);
							/* translators: %1$s: Plan name. */
							/* translators: %2$s: Renew date. */
							/* translators: %3$s: Plane price. */
							printf(__('Your plan <i>"%1$s"</i> will automatically renew the <i>%2$s</i> for the price of <b>%3$s</b>.', "spoki"), $this->options['account_info']['plan']['name'], $plan_end ? $plan_end : '--', $price);
						} ?>
                    </span>
                    <p class="note">
                        <small>
                ...
                <div class="spoki-card-body d-flex flex-direction-xs-column">
                    ...
                    <div class="spoki-statistic <?php if (!$has_abandoned_carts) echo 'disabled' ?>">
                        ...
                        <div>
                            ...
                            <p class="subtitle">
                                <small>
									<?php _e("Total recovered orders", "spoki") ?>.
                                </small>
                            </p>
							<?php if (!$has_abandoned_carts): ?>
                                <a href="?page=<?php echo urlencode(SPOKI_PLUGIN_NAME) ?>&tab=abandoned-carts">
                ...
                <div class="spoki-card-body d-flex flex-direction-xs-column">
                    <div class="spoki-statistic <?php if (!$has_abandoned_carts) echo 'disabled' ?>">
                        <span class="coins small">
                            <?php echo esc_attr(get_woocommerce_currency_symbol()) . esc_attr(number_format_i18n($abandoned_report['revenue'], 2)); ?>
                        </span>
                        <div>
                    ...
                    <div class="spoki-statistic <?php if (!$has_abandoned_carts) echo 'disabled' ?>">
                        <span class="coins small">
                            <?php echo esc_attr(get_woocommerce_currency_symbol()) . esc_attr(number_format_i18n($recovered_report['revenue'], 2)); ?>
                        </span>
                        <div>
                    ...
                    <div class="spoki-statistic <?php if (!$has_abandoned_carts) echo 'disabled' ?>">
                        ...
                        <div>
                            <h3 class="label"><?php _e("Recovery Rate", "spoki") ?></h3>
                            <p class="subtitle">
                                <small>
			...
			<?php if (isset($this->options['account_info']['statistics']['notifications'])) : ?>
                ...
                <div class="spoki-card">
                    ...
                    <div class="spoki-card-body d-flex flex-direction-xs-column">
                        <div class="spoki-statistic <?php if (!$has_leave_review_notification) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['notifications']['leave_review'] ?? '0' ?>
                            </span>
                            <div>
                                <h3 class="label">
                        ...
                        <div class="spoki-statistic <?php if (!$has_order_status_notifications) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['notifications']['order_status'] ?? '0' ?>
                            </span>
                            <div>
                        ...
                        <div class="spoki-statistic <?php if (!$has_order_note_added_notification) echo 'disabled' ?>">
                                <span class="coins">
                                    <?php echo $this->options['account_info']['statistics']['notifications']['tracking_info'] ?? '0' ?>
                                </span>
                            <div>
                ...
                <div class="spoki-card">
                    ...
                    <div class="spoki-card-body d-flex flex-direction-xs-column">
                        <div class="spoki-statistic <?php if (!$has_order_created_to_seller_notification) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['notifications']['order_created_to_seller'] ?? '0' ?>
                            </span>
                            <div>
                                <h3 class="label">
                ...
                <div class="spoki-card w-50 w-xs-100">
                    ...
                    <div class="spoki-card-body">
                        <div class="spoki-statistic <?php if (!$has_fixed_support_button) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['widget_fixed'] ?? '0' ?>
                            </span>
                            <div>
                ...
                <div class="spoki-card w-50 w-xs-100">
                    ...
                    <div class="spoki-card-body">
                        <div class="spoki-statistic <?php if (!$has_product_item_listing_button) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['widget_woo_shop'] ?? '0' ?>
                            </span>
                            <div>
                                <h3 class="label"><?php _e("Clicks", "spoki") ?></h3>
                                <p class="subtitle">
                ...
                <div class="spoki-card w-50 w-xs-100">
                    ...
                    <div class="spoki-card-body">
                        <div class="spoki-statistic <?php if (!$has_single_product_button) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['widget_woo_item'] ?? '0' ?>
                            </span>
                            <div>
                                <h3 class="label"><?php _e("Clicks", "spoki") ?></h3>
                                <p class="subtitle">
                ...
                <div class="spoki-card w-50 w-xs-100">
                    ...
                    <div class="spoki-card-body">
                        <div class="spoki-statistic <?php if (!$has_cart_button) echo 'disabled' ?>">
                            <span class="coins">
                                <?php echo $this->options['account_info']['statistics']['widget_woo_cart'] ?? '0' ?>
                            </span>
                            <div>
                                <h3 class="label"><?php _e("Clicks", "spoki") ?></h3>
                                <p class="subtitle">
