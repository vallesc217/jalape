(function ($) {
    ...
    var spoki_cart_abandonment = {
        ...
        _getCheckoutData: function () {
            ...
            var spoki_phone = jQuery("#billing_phone").val();
            ...
            if (typeof spoki_phone === 'undefined' || spoki_phone === null) { //If phone number field does not exist on the Checkout form
                spoki_phone = '';
            }

            clearTimeout(timer);

            if (!(atposition < 1 || dotposition < atposition + 2 || dotposition + 2 >= spoki_email.length) || spoki_phone.length >= 1) { //Checking if the email field is valid or phone number is longer than 1 digit
