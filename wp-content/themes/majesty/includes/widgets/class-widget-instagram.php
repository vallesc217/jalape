...
class Sama_Widget_Instagram extends WP_Widget {
	...
	function widget ($args, $instance) {
		...
		if ( false !== $instagram ) {
			...
			foreach( $instagram['data'] as $image ) {
				...
				if ( $displayed <= $count ) {
			?>
					<li><a data-rel="prettyPhoto[instagram-feed-<?php echo $rand; ?>]" href="<?php echo esc_url($image['images']['standard_resolution']['url']);?>"><img src="<?php echo esc_url($image['images']['thumbnail']['url']); ?>"></a></li>
		<?php
				}
			}
			echo '</ul>';
	...
	public function get_instagrams( $args = array() ) {
		...
		if ( false === ( $instagrams = get_transient( $this->_transient_key ) ) ) {
			...
			$api_url = 'https://api.instagram.com/v1/users/1187916812/media/recent/?access_token='. $access_token;
	...
	?>
		...
		<p><label for="<?php echo $this->get_field_id('access_token'); ?>"><?php esc_html_e( 'Access Token:', 'theme-majesty'); ?> </label><input class="widefat" type="text" name="<?php echo $this->get_field_name('access_token'); ?>" id="<?php echo $this->get_field_id('access_token'); ?>" value="<?php echo esc_attr($instance['access_token']); ?>" size="20" />
		<br/><span>Please read this <a href="http://docs.eyethemes.com/themes/majesty/#instagramwidget" target="_blank">documentation</a>.</span>
		</p>
