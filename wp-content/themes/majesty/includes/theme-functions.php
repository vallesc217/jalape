...
if ( ! function_exists ( 'sama_enqueue_scripts_styles' ) ) {
	function sama_enqueue_scripts_styles() {
		...
		wp_register_style('style', 				get_stylesheet_uri(), array('bootstrap-min'), SAMA_THEME_VER);
		wp_register_style('style-rtl', 			SAMA_THEME_URI .'/css/rtl/style-rtl.min.css', '', SAMA_THEME_VER);
		wp_register_style('themegrid', 			SAMA_THEME_URI .'/css/theme-grid.min.css', '', SAMA_THEME_VER);
		wp_register_style('responsive', 		SAMA_THEME_URI .'/css/main-responsive.min.css', '', SAMA_THEME_VER);
		wp_register_style('main-responsive-rtl', SAMA_THEME_URI .'/css/rtl/main-responsive-rtl.min.css', '', SAMA_THEME_VER);
		wp_register_style('font-awesome', 		SAMA_THEME_URI .'/css/font-awesome.min.css', '', '4.3.0');
		wp_register_style('animate', 			SAMA_THEME_URI .'/css/animate.min.css', '', SAMA_THEME_VER);
		wp_register_style('bootstrap-min', 		SAMA_THEME_URI .'/css/bootstrap/css/bootstrap.min.css', '', '3.3.4');
		wp_register_style('bootstrap-rtl', 		SAMA_THEME_URI .'/css/rtl/bootstrap-rtl.min.css', '', SAMA_THEME_VER);
		//	Enqueue styles
		...
		if( is_rtl() ) {
			wp_enqueue_style('main-responsive-rtl');
		}
		// Register Scripts
		/*
		...
		//wp_register_script('theme-script',  	SAMA_THEME_URI .'/js/theme-scripts.js', array('jquery'), SAMA_THEME_VER, true);
		...
		wp_localize_script( 'theme-script', 'sama_ajax_load_more_posts', array(
			'ajaxurl'			=> admin_url( 'admin-ajax.php'),
			'eyeajaxlivesearch'	=> wp_create_nonce('eye-ajax-live-search-nonce')
		));
...
add_action( 'wp_ajax_product_search', 'sama_ajax_live_search' );
add_action( 'wp_ajax_nopriv_product_search', 'sama_ajax_live_search' );

if ( ! function_exists ( 'sama_ajax_live_search' ) ) {
	/**
	 * get posts for ajax search.
	 *
	 * @since 1.0.0
	 *
	 * @return array string to display html content.
	 */
	function sama_ajax_live_search() {
		$nonce = $_REQUEST['noncesearch'];
		if ( ! wp_verify_nonce( $nonce, 'eye-ajax-live-search-nonce' ) ) {
			//die( 'Security check' );
			$post_data['label'] 	= esc_html__('Security check', 'theme-majesty');
			$post_data['link'] 		= '';
			$post_data['img'] 		= '';
			$post_data['price']		= '';
			$post_data['rating']	= '';
			$post_data['search'] 	= '';
			wp_die(json_encode($post_data));
		}
		
		$term = trim(stripslashes ($_POST['search']) );
		...
		if( $products->have_posts() ) {
			while( $products->have_posts() ) {
				...
				$post_data['img'] 		= get_the_post_thumbnail_url( '', 'majesty-thumb-100');
				$post_data['price']		= $product->get_price_html();
				$post_data['rating']	= wc_get_rating_html( $product->get_average_rating() );
				$post_data['search'] 	= '';
				$suggestions[] 			= $post_data;
			}
			$post_data['label'] 	= esc_html__( 'View all results', 'theme-majesty');
			$post_data['link'] 		= esc_url( site_url() .'?s='. $term .'&post_type=product' );
			$post_data['img'] 		= '';
			$post_data['price']		= '';
			$post_data['rating']	= '';
			...
			$post_data = array();
			$post_data['label'] 	= esc_html__( 'No Posts found', 'theme-majesty');
			$post_data['link'] 		= '';
			$post_data['img'] 		= '';
			$post_data['search'] 	= '';
			$suggestions[] 			= $post_data;
...
if ( ! function_exists ( 'sama_post_thumbnail_sizes_attr' ) ) {
	function sama_post_thumbnail_sizes_attr($attr, $attachment, $size) {
		
		$css_classes = explode(' ', $attr['class'] );
		
		if ( $size === 'majesty-shop-400' ) {
			$attr['srcset'] = '';
			$attr['sizes'] = '';
		}
		return $attr;
...
add_filter('wp_get_attachment_image_attributes', 'sama_post_thumbnail_sizes_attr', 100 , 3);
?>