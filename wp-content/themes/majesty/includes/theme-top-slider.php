...
?>
			<section id="top-yt-bg-player" class="top-slider fullheight wrap-top-yt-bg-player">
				<div class="yt-bg-player fullheight" <?php echo esc_attr( $attr ); ?>>
					<?php if( ! empty( $overlay ) ) { ?>
						<div class="bg-transparent <?php echo esc_attr( $overlay ); ?>">
					<?php } ?>
							<?php
								if( isset( $content ) ) {
									$content = do_shortcode( force_balance_tags( $content ) );
...
<?php		
		...
		$attr 		= ' data-video='. esc_url( $vimeo_args['url'] ) .' data-loop='. esc_attr( $loop ) .' data-autoplay='. esc_attr( $autoplay ) .'';
		if( ! empty( $poster ) ) {
			$attr 		.= ' data-poster='. esc_url( $poster ) .'';
		}
?>
