...
/**
 * Force Visual Composer to hide certain WPBakery Page Builder settings tabs
 */
...
function sama_get_buttons_color() {
	$colors_arr = array(
...
function sama_vc_get_string_for_tab_content() {
	return array( esc_html__('By SamaThemes', 'theme-majesty'), esc_html__('Content', 'theme-majesty') );
}

function sama_vc_get_text_align() {
	return array(
...
function sama_vc_map_add_el_css_field() {
	return array(
		'type' 			=> 'textfield',
...
function sama_vc_map_add_default_title() {
	return array(
				'type' 			=> 'textfield',
...
function sama_vc_map_add_title_on_admin_only() {
	return array(
			'type' => 'textfield',
			'heading' => esc_html__('Title', 'theme-majesty'),
...
function sama_vc_map_add_field_orderby() {
	return array(
				'type' 			=> 'dropdown',
...
function sama_vc_map_add_field_owl_attached() {
	return array(
			'type' => 'attach_images',
...
function sama_vc_map_add_field_owl_pagination() {
	return array(
			'type' 			=> 'dropdown',
...
function sama_vc_map_add_field_order_way() {
	return array(
			'type' 			=> 'dropdown',
...
function sama_vc_map_add_field_woocommerce_perpage( $num = 4) {
	return array(
				'type' 			=> 'textfield',
...
function sama_vc_map_add_field_woocommerce_cats() {
	return array(
				'type' 			=> 'checkbox',
...
function sama_icons_moon_settings_field($settings, $value) {
	...
	$return 		= '<div class="icongroup"><input type="hidden" name="'.$settings['param_name'].'" class="wpb_vc_param_value wpb-textinput '.$settings['param_name'].' '.$settings['type'].'_field vc-icon-select" value="'.$value.'">';
	$icon_value 	= $value;			
...
vc_map( array(
	...
	'admin_label' 		=> true,
	'category' 			=>	sama_vc_get_string_for_tab_content(),
	'params' 			=> array(
		array(
		  'type' 			=> 'dropdown',
...
vc_map( array(
	...
	'category'			=>	sama_vc_get_string_for_tab_content(),
	'params' 			=> array(
		sama_vc_map_add_title_on_admin_only(),
		array(
...
vc_map( array(
    ...
    'params' 		=> array(
		...
		array(
			...
			'description' 	=> sama_vc_get_string_for_tab_content(),
...
vc_map( array(
	...
	'params' => array(
		...
		sama_vc_map_add_field_owl_attached(),
		array(
			'type' 			=> 'textfield',
		...
		sama_vc_map_add_field_owl_pagination(),
		array(
			'type' 			=> 'textfield',
...
if ( class_exists('woocommerce') ) {
	...
	vc_map( array(
		...
		'category'		=>	sama_vc_get_string_for_tab_content(),
		'params' 		=> array(
			sama_vc_map_add_field_woocommerce_cats(),
			array(
				'type' 			=> 'textfield',
			...
			sama_vc_map_add_field_woocommerce_perpage(4),
			sama_vc_map_add_field_orderby(),
			sama_vc_map_add_field_order_way(),
			array(
				'type' 			=> 'textfield',
				'heading' 		=> esc_html__( 'Excerpt Length', 'theme-majesty' ),
				'value' 		=> 13,
	...
	vc_map( array(
		...
		'params' 		=> array(
			...
			sama_vc_map_add_field_woocommerce_perpage(12),
			sama_vc_map_add_field_orderby(),
			sama_vc_map_add_field_order_way(),
			sama_vc_map_add_el_css_field()
		)
