...
if( ! empty ( $icon_type ) && $icon_type == 'image' && ! empty( $image ) ) {
	$img_url = wp_get_attachment_image_src($image, 'full');
	$output .= '<p class="heading-img '. esc_attr( trim( $text_align ) ) .'"><img src="'. esc_url( $img_url[0] ) .'" alt="'. esc_attr( strip_tags( $title ) ) .'"></p>';
} elseif( ! empty ( $icon_type ) ) {
