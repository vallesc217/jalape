...
foreach( $cats as $cat ) {
	...
	if ( ( $products ) && $products->have_posts() ) {
		...
		?>
				<div class="item">
					<?php
						$image = preg_replace( '/(width|height)="\d*"\s/', "", $product->get_image('majesty-woo-slider-large', array( 'class' => 'lazyOwl' )) );
						echo $image;
					?>
					<div class="item_desc">
						...
						<a class="btn btn-gold" href="<?php echo esc_url( get_permalink( $product->get_id() ) ); ?>" title="<?php echo esc_attr( $product->get_title() ); ?>"><i class="fa fa-link"></i></a>
						</div>
					</div>
                </div>
		<?php
