(function( $ ) {
   "use strict";
        ...
        parallax: function(){
			if( $parallaxEl.length > 0 ){
				$parallaxEl.each(function(){
					...
					$(this).paroller({ factor: 0.5, factorXs: 0.2, factorSm: 0.3, type: 'background' });
				});
			}
        },
        animations: function(){   
            ...
            if( $youTubeBg.length > 0 ){
                $youTubeBg.each( function(){
					...
					/*if( ytbgContainer ) { 
						...
						if( $(this).parent().hasClass('wrap-top-yt-bg-player') ) {
							ytbgContainer = '#top-yt-bg-player';
						} else {
							ytbgContainer = 'self';
						}
					}*/
					/*if( $(this).closest('.wrap-top-yt-bg-player') ) {
						ytbgContainer = 'wrap-top-yt-bg-player';
					} else {
						ytbgContainer = 'self';
					}
					*/
                    if( !ytbgContainer ) { ytbgContainer = 'self'; }
					//if( !ytbgContainer ) { ytbgContainer = $(this).parent(); }
					...
					element.mb_YTPlayer({
							...
							showYTLogo: false,
							useOnMobile: true,
							mobileFallbackImage: ytbgPoster,
	...
	function add_mins_plus_to_cart() {
		//plugin bootstrap minus and plus
		//http://jsfiddle.net/laelitenetwork/puJ6G/
		
		if( $('.quantity').length > 0 && ! $('.add-wrap-qty').length > 0 ) {
			
			if( $('.quantity').find('input[type=number]').length > 0 ) {
			
				$('.quantity').wrap('<div class="input-group plus-minus"></div>');
				$('.input-group.plus-minus').wrap('<div class="wrap-qty add-wrap-qty"></div>');
				
				$(".wrap-qty .input-group").prepend('<span class="input-group-btn"><button type="button" class="btn btn-default btn-number btn-minus" data-type="minus"> <span class="fa fa-minus"></span> </button></span>');
				$(".wrap-qty .input-group").append('<span class="input-group-btn"><button type="button" class="btn btn-default btn-number btn-plus" data-type="plus"> <span class="fa fa-plus"></span> </button></span>');
			}
		...
		$('.btn-number').click(function(e){
			...
			var input = $(this).closest('.plus-minus').find('.input-text');
			var currentVal = parseInt(input.val());
			if (!isNaN(currentVal)) {
				if(type == 'minus') {
					var minValue = parseInt(input.attr('min'));
					if( typeof minValue === undefined || minValue === false || isNaN(minValue)) {
						minValue = 1;
					}
					if(currentVal > minValue) {
						input.val(currentVal - 1).change();
						$(this).closest('.plus-minus').find('.btn-plus').removeAttr('disabled', true);
					} 
					...
					if( typeof maxValue === undefined || maxValue === false || isNaN(maxValue) ) {
						maxValue = 9999999999999;
					}
					if(currentVal < maxValue) {
						input.val(currentVal + 1).change();
						$(this).closest('.plus-minus').find('.btn-minus').removeAttr('disabled', true);
					}
		...
		$('.quantity input[type=number]').focusin(function(){
		...
		$(".quantity input[type=number]").keydown(function (e) {
	...
	$(".menu-search-icon", this).on("click", function(e){
		e.preventDefault();
	...
	eye_ajax_search();
	function eye_ajax_search() {
		$(".ajax-search").autocomplete({
			source: function(name, response) {
				$.ajax({
					type: 'POST',
					dataType: 'json',
					url: sama_ajax_load_more_posts.ajaxurl,
					data: {
						'action': 		'product_search',
						'noncesearch' : sama_ajax_load_more_posts.eyeajaxlivesearch,
			...
			search: function(event, ui) { 
			   $(this).closest('.input-group').addClass('loading');
			},
			response: function(event, ui) {
			   $(this).closest('.input-group').removeClass('loading');
			},
			open: function(e) {
				$('ul.ui-autocomplete').removeClass('themeanimated themefadeOutUp').addClass('themeanimated themefadeInDown');
			},
			close: function () { 
				$('ul.ui-autocomplete').removeClass('themeanimated themefadeInDown').css('display','block').addClass('themeanimated themefadeOutUp').delay(1000).css('display','none'); 
			},
			delay: 500,
			minLength: 3,
			...
			if (item.search != '') {
				return $( "<li>" )
					.append( '<span class="search-page"><a href="' + item.link + '">' + item.label + '</a>').appendTo(ul);
			} else {
				if (item.link != '') {
					return $( "<li>" )
					.append( '<span class="thumb"><a href="' + item.link + '"><img src="'+ item.img +'"></a></span><span class="itemdetails"><span class="itemtitle"><a href="' + item.link + '">' + item.label + '</a></span><span class="price">'+ item.price +'</span><span class="item-rate">'+ item.rating +'</span></span>').appendTo(ul);
				} else {
					return $( "<li>" )
					.append( item.label )
					.appendTo(ul);
